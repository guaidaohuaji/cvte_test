...
int main(void)
{
    ...
    /* A formal health trip must force 0% and block every restart path. */
    assert(AppFan_TripSafetyFault());
    assert(AppFan_IsSafetyFaultLatched());
    assert(AppFan_IsRestartInhibited());
    assert(applied_duty_from_ccr() == 0U);
    assert(AppFan_GetSnapshot(&snapshot));
    assert(!snapshot.enabled);
    assert(snapshot.state == APP_FAN_STATE_SAFETY_LOCKED);
    assert(snapshot.applied_duty_x100 == 0U);
    assert(!AppFan_SetEnabled(true, 3000U));

    /* Clearing a fault deliberately does not authorize a restart. */
    assert(AppFan_ClearSafetyFault());
    assert(!AppFan_IsSafetyFaultLatched());
    assert(AppFan_IsRestartInhibited());
    assert(!AppFan_SetEnabled(true, 3000U));
    assert(applied_duty_from_ccr() == 0U);

    assert(AppFan_AuthorizeRestart());
    assert(!AppFan_IsRestartInhibited());
    fake_tick = 6000U;
    assert(AppFan_SetEnabled(true, 3000U));
    assert(applied_duty_from_ccr() == 10000U);

    puts("fan startup and safety-lock tests passed");
    return 0;
}
