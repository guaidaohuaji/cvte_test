...
static void test_safety_latch_and_explicit_restart(void)
{
    AppManualFanControlSnapshot s;
    uint32_t commands_before;
    ...
    s = snapshot();
    assert(s.mode == APP_MANUAL_FAN_MODE_OFF);
    assert(s.control_state == APP_MANUAL_FAN_CTRL_SAFETY_LOCKED);
    assert(fan_command_count == commands_before);
    assert(AppManualFanControl_SetDuty(3000U) == APP_MANUAL_FAN_RESULT_HW_ERROR);

    /* A clear leaves restart inhibited. The next explicit manual command is
     * the authorization event and may start the fan again. */
    fake_health_latched = false;
...
int main(void)
{
    ...
    test_safety_latch_and_explicit_restart();
