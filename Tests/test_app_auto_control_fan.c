...
static bool fake_health_latched;
...
int main(void)
{
    ...
    fake_health_latched = false;
    fake_restart_inhibited = false;

    fake_ntc.state = APP_NTC_STATE_OK;
    fake_ntc.range_status = APP_NTC_RANGE_IN_RANGE;
    fake_ntc.sensor_measurement_valid = true;
    /* NTC Step27 can report -40°C, but AUTO must retain its -25°C cold endpoint. */
    fake_ntc.temp_centi_c = -4000;
    fake_ntc.control_temp_centi_c = -4000;
    ...
    assert(snapshot.target_fan_rpm == APP_AUTO_FAN_MIN_RPM);
    assert(snapshot.target_damper_steps == APP_AUTO_DAMPER_COLD_STEPS);
    ...
    /* NTC Step27 can report 120°C, but AUTO must retain its 60°C hot endpoint. */
    fake_ntc.control_temp_centi_c = 12000;
    fake_ntc.temp_centi_c = 12000;
    commands_before = fan_command_count;
    run_at(8000U);
    snapshot = auto_snapshot();
    ...
    /* A health fault stops only the fan side of AUTO; no restart command is
     * issued while either the latch or post-clear inhibit remains active. */
    fake_health_latched = true;
    ...
    run_at(9000U);
    snapshot = auto_snapshot();
    assert(snapshot.fan_control_state == APP_AUTO_FAN_CTRL_SAFETY_LOCKED);
    assert(fan_command_count == commands_before);

    fake_health_latched = false;
    run_at(10000U);
    assert(auto_snapshot().fan_control_state == APP_AUTO_FAN_CTRL_SAFETY_LOCKED);
    assert(fan_command_count == commands_before);

    /* Re-sending AUTO after clear is the explicit restart authorization. */
    assert(AppAutoControl_SetMode(APP_AUTO_MODE_AUTO) == APP_AUTO_SET_MODE_OK);
    assert(!fake_restart_inhibited);
    run_at(10001U);
    assert(fan_command_count == commands_before + 1U);

    puts("auto fan closed-loop and safety-lock tests passed");
    return 0;
}
