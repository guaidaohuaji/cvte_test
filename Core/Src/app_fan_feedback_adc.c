/**
 * @file app_fan_feedback_adc.c
 * @brief 风机 FG 正式结果仲裁层（实现文件）。
 *
 ...
 * @param prev_cand 见调用点；该参数只在本次调用期间有效。
 * @param new_cand 见调用点；该参数只在本次调用期间有效。
 * @note 文件内部辅助函数，不属于对外 API。
 */
...
/**
 * @brief 根据最新输入更新内部状态。
 * @param flt_min 见调用点；该参数只在本次调用期间有效。
 * @param flt_max 见调用点；该参数只在本次调用期间有效。
 * @param flt_count 见调用点；该参数只在本次调用期间有效。
 * @note 文件内部辅助函数，不属于对外 API。
 */
...
/**
 * @brief 将仲裁选中的 BPF 或 Legacy 结果复制到正式快照，并在来源改变时累计切换次数。
 * @note 文件内部辅助函数，不属于对外 API。
 */
...
/**
 * @brief 清除内部临时状态，使后续流程重新同步。
 * @param reset_shadow_bpf 见调用点；该参数只在本次调用期间有效。
 * @note 文件内部辅助函数，不属于对外 API。
 */
 ...
 * @param rise_idx 见调用点；该参数只在本次调用期间有效。
 * @note 文件内部辅助函数，不属于对外 API。
 */
