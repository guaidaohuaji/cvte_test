...
| PE8 | GPIO输出 | LED | 低有效 |
| PE9 | TIM1_CH1 | PWM输入 | AF1，TIM1 PWM Input + DMA自动量程 |
...
| 定时器 | 用途 | 关键配置 |
|---|---|---|
| TIM1 | PWM输入捕获 | 运行时FAST/MEDIUM/SLOW三档PSC；PWM Input reset mode；DMA2 Stream6 Channel0；ICFilter=2 |
...
USART1              10
USART6              11
TIM1_CC             12（Step23默认引擎不启用CC中断，保留兼容）
TIM1_UP_TIM10       12（共享TIM10；Step23默认TIM1不启用UPDATE中断）
DMA2_Stream0        13
TIM6_DAC            14
SysTick             15
```
