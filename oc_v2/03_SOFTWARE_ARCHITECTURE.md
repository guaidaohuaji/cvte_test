...
```text
GPIO → DMA → USART1 → USART6 → TIM4 → TIM1 → ADC1 → TIM10 → TIM2
Master额外TIM6
→ AppUart → AppOneWireUart → AppOneWire → LED → PWM输出 → PWM输入
→ NTC → Fan → ADC Scan → BPF → Damper → AUTO → Manual Fan → Fan Health
```

HAL回调归属：

```text
main.c: UART RX/TX/Error、TIM捕获/周期分发
app_adc_scan.c: ADC DMA half/full callbacks
```

同一种 HAL weak callback 不得在多个文件重复强定义。
